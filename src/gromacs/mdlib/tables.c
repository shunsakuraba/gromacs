                       ...
                       gmx_bool b14only)
{
    ...
    for (i = td->nx0; (i < td->nx); i++)
    {
        ...
        switch (tp)
        {
            ...
            case etabZQ:
                if(r < rc) {
                    Vtab  = gmx_erfc(fr->zd_alpha * r) / r - gmx_erfc(fr->zd_alpha * rc) / rc + fr->k_zmm_2 * (r * r - rc * rc) + fr->k_zmm_4 * (pow(r, 4) - pow(rc, 4));
                    Ftab = gmx_erfc(fr->zd_alpha * r) / r2 + fr->zd_alpha * M_2_SQRTPI * exp(- fr->zd_alpha*fr->zd_alpha * r2) / r - 2 * fr->k_zmm_2 * r - 4 * fr->k_zmm_4 * r * r * r;
                }
                else
                {
                    Vtab = 0;
